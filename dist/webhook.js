...
export async function POST(req) {
    ...
    const headerSecret = req.headers.get('x-cms-webhook-secret') ??
        req.headers.get('authorization')?.replace(/^Bearer\s+/i, '');
    const expectedSecret = (typeof process !== 'undefined' ? process.env?.CMS_WEBHOOK_SECRET : undefined) ||
    ...
    if (!isValid) {
        return NextResponse.json({ ok: false, message: 'Unauthorized: Invalid HMAC signature or webhook secret' }, { status: 401 });
    }
    let payload = {};
